...
oddPrediction <-
  function(dateStart,
    ...
    for (date in as.integer(gsub("-", "", as.character(seq(
      ymd(dateStart), ymd(dateEnd), 1
    ))))) {
      print(date)
      if (dim(dataset[dataset$Date == date, ])[1] != 0) {
        ...
        dataset <-
          updateFile1(dataset, date, eloDay, homeAwayDay, restDay)
        
        if (everyDay == TRUE) {
          neuralNetworkDay <- trainDay(dataset, date, realCondition)
        }
        else{
...
eloProbability <- function(dataset, date) {
  ...
  for (i in seq(lengthCDS, 2,-2)) {
    ...
    names(bufferHome) <-
      c("indexTeam", "indexOpponent", "eloOpponent", "percScore")
    bufferAway <- bufferHome
    
    bufferHome$indexTeam <-
      which(pastDataset$Team == currentTeamHome)[1:10]
    bufferHome$indexOpponent <-
      bufferHome$indexTeam - ((bufferHome$indexTeam + 1) %% 2) + (bufferHome$indexTeam %% 2)
    bufferHome$eloOpponent <-
      pastDataset$prevEloRanking[bufferHome$indexOpponent]
    bufferHome$percScore <-
      pastDataset$Score[bufferHome$indexTeam] / (pastDataset$Score[bufferHome$indexTeam] +
                                                   pastDataset$Score[bufferHome$indexOpponent])
    indEloHome <-
      sum(alpha * bufferHome$eloOpponent * bufferHome$percScore) / (pastDataset$eloRanking[which(pastDataset$Team == currentTeamAway)[1]] *
                                                                      sum(alpha))
    
    bufferAway$indexTeam <-
      which(pastDataset$Team == currentTeamAway)[1:10]
    bufferAway$indexOpponent <-
      bufferAway$indexTeam - ((bufferAway$indexTeam + 1) %% 2) + (bufferAway$indexTeam %% 2)
    bufferAway$eloOpponent <-
      pastDataset$prevEloRanking[bufferAway$indexOpponent]
    bufferAway$percScore <-
      pastDataset$Score[bufferAway$indexTeam] / (pastDataset$Score[bufferAway$indexTeam] +
                                                   pastDataset$Score[bufferAway$indexOpponent])
    indEloAway <-
      sum(alpha * bufferAway$eloOpponent * bufferAway$percScore) / (pastDataset$eloRanking[which(pastDataset$Team == currentTeamHome)[1]] *
                                                                      sum(alpha))
...
eloFunction <- function(x) {
  output <-
    0.004 * x ^ 6 + 1618.7 * x ^ 5 - 0.0007 * x ^ 4 - 98.115 * x ^ 3 - 10 ^
    (-8) * x ^ 2 + 4.3177 * x + 0.5
  return(output)
}
...
HWProbability <- function(dataset, date) {
  ...
  for (i in seq(lengthCDS, 2,-2)) {
    ...
    Home_nbGamesHome <-
      length(
        which(
          pastDatasetHome$Team == currentTeamHome &
            pastDatasetHome$Saison == season
        )
      )
    Away_nbGamesAway <-
      length(
        which(
          pastDatasetAway$Team == currentTeamAway &
            pastDatasetAway$Saison == season
        )
      )
    
    if (Home_nbGamesHome >= 5) {
      winningPercHome <-
        sum(pastDatasetHome$VD[pastDatasetHome$Team == currentTeamHome &
                                 pastDatasetHome$Saison == season]) / Home_nbGamesHome
    }
    else{
      Home_nbGamesHome <-
        length(
          which(
            pastDatasetHome$Team == currentTeamHome &
              pastDatasetHome$Saison == season - 1
          )
        )
      winningPercHome <-
        sum(pastDatasetHome$VD[pastDatasetHome$Team == currentTeamHome &
                                 pastDatasetHome$Saison == season - 1]) / Home_nbGamesHome
    }
    
    if (Away_nbGamesAway >= 5) {
      winningPercAway <-
        sum(pastDatasetAway$VD[pastDatasetAway$Team == currentTeamAway &
                                 pastDatasetAway$Saison == season]) / Away_nbGamesAway
    }
    else{
      Away_nbGamesAway <-
        length(
          which(
            pastDatasetAway$Team == currentTeamAway &
              pastDatasetAway$Saison == season - 1
          )
        )
      winningPercAway <-
        sum(pastDatasetAway$VD[pastDatasetAway$Team == currentTeamAway &
                                 pastDatasetAway$Saison == season - 1]) / Away_nbGamesAway
    }
    
    probaCDHome_num <-
      betas[1] * winningPercHome + betas[2] * (1 - winningPercAway)
    probaCDAway_num <-
      betas[1] * (1 - winningPercHome) + betas[2] * winningPercAway
...
restProbability <- function(dataset, date) {
  ...
  matrixCons <-
    matrix(c(0.5, 0.41, 0.35, 0.59, 0.5, 0.41, 0.65, 0.59, 0.5),
           nrow = 3,
           ncol = 3)
  matrix7days <-
    matrix(
      c(0.5, 0.4219, 0.4048, 0.5781, 0.5, 0.4018, 0.5952, 0.5982, 0.5),
      nrow = 3,
      ncol = 3
    )
  matrixRestDays <-
  ...
  for (i in seq(lengthCDS, 2,-2)) {
    ...
    consecutiveGamesHome <-
      dim(pastDataset[pastDataset$Team == currentTeamHome &
                        pastDataset$Date == as.integer(gsub("-", "", as.character(dateYMD - 1))), ])[1] +
      dim(pastDataset[pastDataset$Team == currentTeamHome &
                        pastDataset$Date == as.integer(gsub("-", "", as.character(dateYMD - 1))), ])[1] *
      dim(pastDataset[pastDataset$Team == currentTeamHome &
                        pastDataset$Date == as.integer(gsub("-", "", as.character(dateYMD - 2))), ])[1]
    
    consecutiveGamesAway <-
      dim(pastDataset[pastDataset$Team == currentTeamAway &
                        pastDataset$Date == as.integer(gsub("-", "", as.character(dateYMD - 1))), ])[1] +
      dim(pastDataset[pastDataset$Team == currentTeamAway &
                        pastDataset$Date == as.integer(gsub("-", "", as.character(dateYMD - 1))), ])[1] *
      dim(pastDataset[pastDataset$Team == currentTeamAway &
    ...
    restingDaysHome <-
      dateYMD - 1 - ymd(pastDataset$Date[which(pastDataset$Team == currentTeamHome)[1]])
    restingDaysAway <-
      dateYMD - 1 - ymd(pastDataset$Date[which(pastDataset$Team == currentTeamAway)[1]])
    
    nbGames7Home <-
      dim(pastDataset[pastDataset$Team == currentTeamHome &
                        pastDataset$Date >= as.integer(gsub("-", "", as.character(dateYMD - 7))), ])[1]
    nbGames7Away <-
      dim(pastDataset[pastDataset$Team == currentTeamAway &
                        pastDataset$Date >= as.integer(gsub("-", "", as.character(dateYMD - 7))), ])[1]
    
    pourcConsHome <-
      matrixCons[consecutiveGamesHome + 1, consecutiveGamesAway + 1]
    pourcConsAway <-
      matrixCons[consecutiveGamesAway + 1, consecutiveGamesHome + 1]
    ...
    if (restingDaysHome > 3 || restingDaysAway > 3) {
      ...
      pourcRestHome <-
        matrixRestDays[restingDaysHome + 1, restingDaysAway + 1]
      pourcRestAway <-
        matrixRestDays[restingDaysAway + 1, restingDaysHome + 1]
    }
    
    if (nbGames7Home > 4 ||
        nbGames7Home < 2 || nbGames7Away > 4 || nbGames7Away < 2) {
      pourc7Home <- 0.5
      pourc7Away <- 0.5
    }
    else{
    ...
    restProbabilityHomeNum <-
      gammas[1] * pourcRestHome + gammas[2] * pourcConsHome + gammas[3] * pourc7Home
    restProbabilityAwayNum <-
      gammas[1] * pourcRestAway + gammas[2] * pourcConsAway + gammas[3] * pourc7Away
    
    restProbabilityHome <-
      restProbabilityHomeNum / (restProbabilityHomeNum + restProbabilityAwayNum)
    restProbabilityAway <-
      restProbabilityAwayNum / (restProbabilityHomeNum + restProbabilityAwayNum)
...
updateFile1 <-
  function(dataset,
    ...
    dateLast <-
      which(dataset$Date == date)[length(which(dataset$Date == date))]
...
trainDay <- function(dataset, date, realCondition) {
  size <- 5
  decay <- 0.0001
  maxit <- 10000
  repeats <- 50
  ...
  if (realCondition == TRUE) {
    dataset_train <-
      dataset[dataset$Periode == dataset$Periode[dataset$Date == date][1] &
                dataset$Date < date, c("Odd12", "ProbaCD", "ProbaDE", "ProbaRest")]
    output <-
      dataset[dataset$Periode == dataset$Periode[dataset$Date == date][1] &
                dataset$Date < date, "VD"]
  }
  else{
    dataset_train <-
      dataset[dataset$Periode == dataset$Periode[dataset$Date == date][1] &
...
predictDay <- function(dataset, date, NN) {
  iter <- length(NN)
  datatest <-
    dataset[dataset$Date == date, c("Odd12", "ProbaCD", "ProbaDE", "ProbaRest")]
